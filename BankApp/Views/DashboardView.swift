                ...
                VStack(spacing: 16) {
                    ForEach(viewModel.deliveryAssignments) { assignment in
                        DeliveryAssignmentRow(
                            ...
                            onAdvance: {
                                viewModel.advanceDeliveryStatus(for: assignment)
                            }
                        )
                    }
                }
...
private struct DeliveryAssignmentRow: View {
    ...
    private var statusAccent: Color {
        ...
        case .scheduled: return .tBankYellow
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 18) {
            ...
            VStack(spacing: 14) {
                DeliveryTimeline(status: assignment.status, accent: statusAccent)
            ...
            if assignment.status.nextStatus != nil, let onAdvance {
                Button(action: onAdvance) {
                    Text(assignment.status.actionTitle)
                        .font(.subheadline.weight(.semibold))
                        .frame(maxWidth: .infinity)
                        .padding(.vertical, 12)
                ...
                .buttonStyle(.plain)
                .accessibilityLabel(assignment.status.actionTitle)
            }
        }
        .padding(20)
        .frame(maxWidth: .infinity, alignment: .leading)
...
private struct DeliveryTimeline: View {
    ...
    private let steps: [CardDeliveryAssignment.Status] = [.scheduled, .enRoute, .delivered]
        ...
        VStack(alignment: .leading, spacing: 6) {
            HStack(alignment: .center, spacing: 0) {
                ForEach(Array(steps.enumerated()), id: \.offset) { index, step in
                    HStack(spacing: 0) {
                        Circle()
                            .fill(index <= status.progressIndex ? accent : Color.tBankSurfaceHighlight)
            ...
            HStack {
                ForEach(steps, id: \.self) { step in
                    Text(label(for: step))
                        .font(.caption2.weight(step == status ? .bold : .semibold))
                        .foregroundColor(step.progressIndex <= status.progressIndex ? accent : .tBankSecondaryText)
    ...
    private func label(for step: CardDeliveryAssignment.Status) -> String {
        switch step {
    ...
    private func alignment(for step: CardDeliveryAssignment.Status) -> Alignment {
        switch step {
        case .scheduled: return .leading
        case .enRoute: return .center
        case .delivered: return .trailing
        }
    }
}
