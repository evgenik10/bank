...
final class SessionManager: ObservableObject {
    ...
    func register(name: String, email: String, password: String, role: User.Role) {
        authError = nil
        authState = .authenticating

        Task {
            do {
                let user = try await backend.register(name: name, email: email, password: password, role: role)
                await startSession(for: user)
            } catch {
                authState = .unauthenticated
                authError = error.localizedDescription
