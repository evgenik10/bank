...
final class DashboardViewModel: ObservableObject {
    ...
    func loadData() async {
        do {
            if user.role == .representative {
                ...
                cards = []
                budgets = []
                let assignments = try await service.fetchDeliveryAssignments()
                deliveryAssignments = sortDeliveries(assignments)
            } else {
    ...
    func advanceDeliveryStatus(for assignment: CardDeliveryAssignment) {
        guard let nextStatus = assignment.status.nextStatus else { return }

        Task {
            do {
                let updated = try await service.updateDeliveryAssignment(assignment, to: nextStatus)
                if let index = deliveryAssignments.firstIndex(where: { $0.id == updated.id }) {
                    deliveryAssignments[index] = updated
                    deliveryAssignments = sortDeliveries(deliveryAssignments)
                }
                errorMessage = nil
            } catch {
                errorMessage = error.localizedDescription
    ...
    private func sortDeliveries(_ assignments: [CardDeliveryAssignment]) -> [CardDeliveryAssignment] {
